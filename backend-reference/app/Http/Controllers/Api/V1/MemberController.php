...
use App\Http\Controllers\Controller; use App\Http\Requests\{StoreMemberRequest,UpdateMemberRequest}; use App\Http\Resources\{MemberResource,MemberDetailResource}; use App\Models\Member; use App\Services\MemberService; use App\Support\ScopesQueryByBranch; use Illuminate\Http\{JsonResponse,Request};
class MemberController extends Controller
{
    use ScopesQueryByBranch;
    public function __construct(private MemberService $memberService) {}
    public function index(Request $request): JsonResponse { $this->authorize('viewAny',Member::class); return response()->json(['data'=>MemberResource::collection($this->scopeToBranch(Member::query(),$request->user())->latest()->paginate($request->integer('per_page',15)))]); }
